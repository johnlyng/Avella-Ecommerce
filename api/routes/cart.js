...
// PUT /api/cart/:token/items/:itemId - Update item quantity
router.put('/:token/items/:itemId', validate('updateCartItem'), async (req, res, next) => {
    try {
...
router.delete('/:token/items/:itemId', validate('removeCartItem'), async (req, res, next) => {
    try {
        const { token, itemId } = req.params;
...
// POST /api/cart/merge - Merge guest cart into user cart
router.post('/merge', validate('mergeCart'), async (req, res, next) => {
    try {
